...
if ($user_type == 'admin') {
    ...
    $sql2 = $conn->prepare("SELECT * FROM `assign` WHERE `isActive` = 1 AND `status` = 'assign' AND `user_id` = ? AND `role` = 'vector'");
    $sql2->execute([$user_id]);
    $tasks2 = $sql2->fetchAll(PDO::FETCH_ASSOC);
    $countVector = count($tasks2);
              ...
              <div class="row form-row mb-3">
                <div class="col-12 col-sm-6 p-2">
                  <div class="form-group">
                    ...
                    <input type="hidden"  name="type" value="addVectorBreak" required>
                    <input type="hidden" id="project_id" name="project_id" value="16"
                      required>
                  </div>
                </div>
                ...
                <div class="row">
                    <div class="col border col-8 p-3" style="width:99%;height: 500px;">
                        <div class="text-center">
                            <button type="button" class="btn bg-white btn1  text-center">Vector Task List Under
                                project</button>
                        </div>
                                            ...
                                            <tbody class="scroll-bar">
                                                ...
                                                foreach ($tasks2 as $task) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td> <?php echo date('j M, Y h:i A', strtotime($task['created_at']) )?></td>
                                                        <th class>
                                                        ...
                                                        <td><span class="close">
                                                                <?php echo str_replace('_', ' ', $taskss['status']) ?> </span>
                                                        </td>
                                                        <td>

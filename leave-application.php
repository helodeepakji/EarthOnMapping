...
$leaves = $conn->prepare("SELECT * FROM `leaves` WHERE `status` = 'pending' ORDER BY `leaves`.`created_at` DESC");
$leaves->execute();
$leaves = $leaves->fetchAll(PDO::FETCH_ASSOC);
$countPending = count($leaves);
                ...
                $user = $user->fetch(PDO::FETCH_ASSOC);
                  ...
                  echo '
                    <tr id="row_'.$leave['leave_id'].'">
                      ...
                      <td> <a class="btn btn-primary" style="margin:0 10px" onclick="approveLeave('.$leave['leave_id'].')">Approve</a><a class="btn btn-danger" onclick="cancelLeave('.$leave['leave_id'].')">Cancel</a></td>
                    </tr>
                  ';
                  $i++;
              }

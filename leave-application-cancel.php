...
$leaves = $conn->prepare("SELECT * FROM `leaves` WHERE `status` = 'cancel' ORDER BY `leaves`.`created_at` DESC");
$leaves->execute();
                ...
                $user = $user->fetch(PDO::FETCH_ASSOC);
                  ...
                  echo '
                    <tr id="row_'.$leave['leave_id'].'">
                      ...
                      <td> <a class="btn btn-primary" style="margin:0 10px" onclick="approveLeave('.$leave['leave_id'].')">Approve</a></td>
                    </tr>
                  ';
                  $i++;
              }
